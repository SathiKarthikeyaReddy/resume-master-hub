...
interface ExecutiveTemplateProps { data: ResumeData; }
  ...
  return (
    <div className="bg-white text-gray-900 min-h-[1123px] w-full max-w-[794px] mx-auto shadow-lg font-['Playfair_Display',_Georgia,_serif] text-sm">
      <header className="bg-slate-900 text-white p-10 relative">
        ...
        <h1 className="text-4xl font-bold tracking-wide text-center mb-4">{personalInfo.fullName || "Your Name"}</h1>
        <div className="flex flex-wrap justify-center items-center gap-x-6 gap-y-2 text-slate-300 text-xs">
          {personalInfo.email && <span className="flex items-center gap-2"><Mail className="w-4 h-4 text-amber-400" />{personalInfo.email}</span>}
          {personalInfo.phone && <span className="flex items-center gap-2"><Phone className="w-4 h-4 text-amber-400" />{personalInfo.phone}</span>}
          {personalInfo.location && <span className="flex items-center gap-2"><MapPin className="w-4 h-4 text-amber-400" />{personalInfo.location}</span>}
          {personalInfo.linkedin && <span className="flex items-center gap-2"><Linkedin className="w-4 h-4 text-amber-400" />{personalInfo.linkedin}</span>}
          {personalInfo.website && <span className="flex items-center gap-2"><Globe className="w-4 h-4 text-amber-400" />{personalInfo.website}</span>}
        </div>
      </header>
      <div className="p-10">
        {summary && (
          <section className="mb-8 text-center">
            <h2 className="text-sm font-bold uppercase tracking-[0.3em] text-slate-400 mb-4 flex items-center justify-center gap-4">
              <span className="w-12 h-px bg-amber-400" />Executive Summary<span className="w-12 h-px bg-amber-400" />
            </h2>
        ...
        {experience.length > 0 && (
          <section className="mb-8">
            <h2 className="text-sm font-bold uppercase tracking-[0.3em] text-slate-400 mb-6 flex items-center gap-4">
              <span className="w-8 h-px bg-amber-400" />Professional Experience
            </h2>
                  ...
                  {exp.bullets?.length > 0 && (
                    <ul className="text-sm text-gray-700 space-y-2 font-['Inter',_sans-serif]">
                      {exp.bullets.filter(b => b.trim()).map((bullet, idx) => (
                        <li key={idx} className="flex gap-3"><span className="text-amber-400 font-bold">•</span>{bullet}</li>
                      ))}
                    </ul>
                  )}
                </div>
        ...
        <div className="grid grid-cols-2 gap-10">
          {education.length > 0 && (
            <section>
              <h2 className="text-sm font-bold uppercase tracking-[0.3em] text-slate-400 mb-4 flex items-center gap-4">
                <span className="w-8 h-px bg-amber-400" />Education
              </h2>
          ...
          {skills.length > 0 && (
            <section>
              <h2 className="text-sm font-bold uppercase tracking-[0.3em] text-slate-400 mb-4 flex items-center gap-4">
                <span className="w-8 h-px bg-amber-400" />Core Competencies
              </h2>
        ...
        <div className="mt-8">
          <CertificationsBlock data={data} variant="executive" />
          <ProjectsBlock data={data} variant="executive" />
          <LanguagesBlock data={data} variant="executive" />
          <VolunteerBlock data={data} variant="executive" />
          <AwardsBlock data={data} variant="executive" />
        </div>
      </div>
      {!hasAnyContent && (
